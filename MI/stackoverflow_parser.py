...
class GroupHandler(xml.sax.ContentHandler):
    ...
    def startElement(self, name, attrs):
        ...
        if attrs.get("PostTypeId") == "1":
            ...
            post["Score"] = attrs.get("Score")
            post["FavoriteCount"] = attrs.get("FavoriteCount") if (
                "FavoriteCount" in attrs.keys()) else '0'
            post["ViewCount"] = attrs.get("ViewCount")
            post["AnswerCount"] = attrs.get("AnswerCount")
            post["CommentCount"] = attrs.get("CommentCount")
            ...
            if len(self.posts) == 100:
                ...
                filename = "SmallData_noText_" + str(self.fileCounter) + ".xml"
                print(filename)
